  ...
  User.findById(req.params.userId)
    ...
    .catch(err => {
      res
        .status(500)
        .json({
          errorMessage: `error retrieving cookbooks by userId: ${req.params.userId}, ${err}`,
        })
    })
})
...
router.post('/:userId/:cookbookId', (req, res) => {
  ...
  User.findById(req.params.userId)
    .populate('cookbooks')
    .then(userData => {
      const cookbookToUpdate = userData.cookbooks.filter(
        cookbook => String(cookbook._id) === String(req.params.cookbookId),
      )
